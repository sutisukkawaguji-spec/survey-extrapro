  ...
  for (var i = 1; i < data.length; i++) {
    try {
      ...
      if (owner === searchUser || sharedWith.indexOf(searchUser) !== -1) {
        ...
        var parsedData = [];
        try { parsedData = JSON.parse(rawData); } catch(je) { parsedData = []; }
        projects.push({
          id: data[i][0],
    ...
    } catch(e) { console.log("Error row " + i + ": " + e.message); }
  }
...
function getSurveyRecords(projectId) {
  ...
  for (var i = 1; i < data.length; i++) {
    if (data[i][1] === projectId) {
      records.push({ record_id: data[i][0], feature_id: data[i][2], surveyor: data[i][3], status: data[i][4], lat: data[i][5], lng: data[i][6], photo_url: data[i][7], note: data[i][8], timestamp: data[i][9] });
    }
  }
...
function login(username, password) {
  ...
  return { status: "error", message: "ชื่อผู้ใช้หรือรหัสผ่านผิด" };
}
...
function register(username, password, name, province) {
  ...
  for (var i = 1; i < data.length; i++) {
    var dbUser = data[i][0] ? data[i][0].toString().trim().toLowerCase() : "";
    if (dbUser === searchUser) return { status: "error", message: "มีชื่อผู้ใช้นี้แล้ว" };
  }
...
function getStaff(province, excludeUser) {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    var dbUser = data[i][0] ? data[i][0].toString().trim().toLowerCase() : "";
    if (dbProv === searchProv && dbUser !== searchEx) staff.push({ username: data[i][0], name: data[i][2] });
  }
